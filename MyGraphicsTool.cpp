...
int	MyGraphicsTool::mWidth = 1920;
int	MyGraphicsTool::mHeight = 1200;
...
void MyGraphicsTool::DrawCurve(const MyArray3f& ctrlpoints, int segs){
	int maxSize;
	glGetIntegerv(GL_MAX_EVAL_ORDER, &maxSize);
	if (ctrlpoints.size() <= maxSize){
		...
		// resample, 07/03/2017
		MyArray3f pts(maxSize);
		int n = ctrlpoints.size();
		for (int i = 0; i < maxSize; i++){
			float f = i / float(maxSize - 1);
			float fn = f*(n-1);
			int fnl = fn;
			if (fnl == n - 1){
				pts[i] = ctrlpoints.back();
			}
			else {
				int fnh = std::min(fnl + 1, n - 1);
				float f = fn - fnl;
				MyVec3f p = f*ctrlpoints[fnh] + (1 - f)*ctrlpoints[fnl];
				pts[i] = p;
			}
		}
		// draw
		glMap1f(GL_MAP1_VERTEX_3, 0.0, 1.0, 3, pts.size(), &(pts[0][0]));
		glEnable(GL_MAP1_VERTEX_3);
		glBegin(GL_LINE_STRIP);
		for (int is = 0; is <= segs; is++)
			glEvalCoord1f((GLfloat)is / segs);
...
void MyGraphicsTool::CopyToTextureImage2D(int x, int y, int width, int height){
	...
	glReadPixels(0, 0, width, height, GL_RGBA, GL_FLOAT, texData);
	for (int i = 0; i < width * height; i++){
		float pixel[4];
		pixel[0] = texData[i * 4 + 0];
		pixel[1] = texData[i * 4 + 1];
		pixel[2] = texData[i * 4 + 2];
		pixel[3] = texData[i * 4 + 3];
		if (texData[i * 4 + 0] <= 0.8){
			texData[i * 4 + 3] = 0;
		}
	}
