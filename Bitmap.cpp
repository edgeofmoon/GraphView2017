...
/*
http://phy.syr.edu/~czhang/code/bmp.c
 --
...
static int hline(int x1, int x2, int y){
	...
	if(y<0  || (size_t)y >= mbmp.height) return 0;
	if(x1>x2)  {x=x1;x1=x2;x2=x;}  /* make sure x1 <= x2 */
	if(x2<0 || (size_t)x1 >= mbmp.width) return 0; /* whole line outa range */
	if(x1<0) x1=0;
	if((size_t)x2 >= mbmp.width) x2=mbmp.width-1;
...
static int vline(int x, int y1, int y2){
	...
	if(x<0 || (size_t)x >= mbmp.width) return 0;
	if(y1 > y2) {y=y1;y1=y2;y2=y;}
	if(y2 < 0 || (size_t)y1 >= mbmp.height) return 0;
	if(y1 < 0) y1=0;
	if((size_t)y2 >= mbmp.height) y2=mbmp.height-1;
...
int line(int x1, int y1, int x2, int y2)
{
	...
	else if(x1 == x2) return vline(x1, y1, y2);
...
MyColor4f getColor(int x, int y){
	int colorIdx = getpixel(x,y);
	MyVec3f color((colorIdx>>16)&0xff,(colorIdx>>8)&0xff,colorIdx&0xff);
	return MyColor4f(color[0]/255.f, color[1]/255.f, color[2]/255.f);
}

float getDistance(const MyScale<float, MyColor4f>& colorScale){
	int numSample = 20;
	float distance = 0;
	for(int i = 0;i<=numSample;i++){
		float pos = i/(float)numSample;
		int posx = mbmp.width/5*pos+0.5f;
		int colorIdx = getpixel(posx,0);
		MyVec3f color((colorIdx>>16)&0xff,(colorIdx>>8)&0xff,colorIdx&0xff);
		color/=255.f;
		MyColor4f sampleColor = colorScale[pos];
		MyVec3f t_color(sampleColor.r, sampleColor.g,sampleColor.b);
		distance += (t_color-color).squared();
	}
	return sqrtf(distance);
}

float getDistance(const MyScale<float, MyColor4f>& colorScale1,
				  const MyScale<float, MyColor4f>& colorScale2){
	int numSample = 40;
	float distance = 0;
	for(int i = 0;i<=numSample;i++){
		float pos = i/(float)numSample;
		MyColor4f color1 = colorScale1[pos];
		MyColor4f color2 = colorScale2[pos];
		MyVec3f color_1(color1.r,color1.g,color1.b);
		MyVec3f color_2(color2.r,color2.g,color2.b);
		distance += (color_1-color_2).squared();
	}
	return sqrtf(distance)/numSample;
}

//#define TEST
int main(void)
{
	int rst = openbmp("colorscale.bmp");
	...
	MyScale<float, MyColor4f> rawColorScale;
	MyArray<MyColor4f> rawHues;
	for(int i = 0;i<mbmp.width;i++){
		rawHues << getColor(i,0);
	}
	rawColorScale.SetDomain(0,1);
	rawColorScale.SetRange(rawHues);
	//rawColorScale.GetRange().SetInterpolationMethod(My1DSampler<MyColor4f>::INTERPOLATE_GAMMA);
	...
	float minGamma;
	for(float gamma = 1.f; gamma <=4; gamma +=0.001f){
		colorScale.GetRange().SetGamma(gamma);
		float t_d = getDistance(colorScale,rawColorScale);
		if(t_d<distance){
			distance = t_d;
			minGamma = gamma;
		}
	...
	for(i=0; i<19; i++) putpixel(i, 5, RGB(60, 80, 180));
	for(i=0; i<19; i++) putpixel(13, i, RGB(80, 255, 80));
